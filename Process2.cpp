...
void DoReceiveCalibrate() {
  ...
  void ProcessIQData2()
  {
    ...
    arm_scale_f32 (cosBuffer3, 0.13, float_buffer_L_EX, 256);  // AFP 10-24-22 Use pre-calculated sin & cos instead of Hilbert
    arm_scale_f32 (sinBuffer3, 0.13, float_buffer_R_EX, 256);  // AFP 10-34-22
    //float rfGainValue;
    if (bands[currentBandA].mode == DEMOD_LSB) {
  ...
  *****/
  void ShowSpectrum2()  //AFP Extensively Modified 3-15-21 Adjusted 12-13-21 to align all elements
  {
    ...
    float adjdB;
    float adjAmplitudeLSB;
    float refAmplitudeLSB;
    float adjAmplitudeUSB;
    float refAmplitudeUSB;
    ...
    for (x1 = 1; x1 < MAX_WATERFALL_WIDTH - 1; x1++)  //AFP, JJP changed init from 0 to 1 for x1: out of bounds addressing in line 112
    {
      ...
      //Serial.print(x1); Serial.print(" y_new= "); Serial.println(y_new);
